    ...
    applyFlagsFromObjectToHtmlOutputData(emoteData : IEmoteDataEntry, emoteObject: EmoteObject, emoteHtml: HtmlOutputData) {
        ...
        if (emoteObject.spin) {
            ...
            if (emoteObject.spin == 'zspin' || emoteObject.spin == 'spin') {
                let diag = Math.sqrt(emoteData.width * emoteData.width + emoteData.height * emoteData.height);
                wrappedEmoteHeight = Math.max(diag, wrappedEmoteHeight);
            }
        }
        if (emoteObject.slide) {
            let slideAnimations : string[] = [];
            let slideSpeed = emoteObject.speed || '8s';
        ...
        if (emoteObject.rotateDegrees) {
            ...
            let rotateHeight = emoteData.width *
        ...
        if (emoteObject.brody) {
            ...
            let brodyHeight = 1.01 * (emoteData.width * Math.sin(10 * Math.PI / 180) + emoteData.height * Math.cos(10 * Math.PI / 180));
        ...
        if (wrappedEmoteHeight) {
            ...
            let offset = Math.floor((wrappedEmoteHeight - emoteData.height) / 2);
