...
var EmoteEffectsModifier = (function () {
    ...
    EmoteEffectsModifier.prototype.applyFlagsFromObjectToEmote = function (emoteObject, $emote) {
        var animations = [];
        var wrapperAnimations = [];
        var transforms = [];
        var needsWrapper = false;
        ...
        if (emoteObject.rotateDegrees) {
            ...
            var rotateHeight = $emote.width() * Math.abs(Math.sin(emoteObject.rotateDegrees * Math.PI / 180)) + $emote.height() * Math.abs(Math.cos(emoteObject.rotateDegrees * Math.PI / 180));
            emoteRoot = this.wrapEmoteHeight($emote, rotateHeight);
        }
